/* ============================================
   AUTHENTICATION SYSTEM — v3.3 (Firebase Google Auth)
   FIX: Removed signInWithRedirect — uses popup only
   Handles popup blockers, storage-partitioned browsers
   ============================================ */
...
const Auth = {
  ...
  async init() {
    ...
    // Handle redirect result (in case of page reload after redirect)
    try {
      ...
      if (error.code && error.code !== 'auth/no-auth-event') {
        console.warn('[Auth] getRedirectResult error (can be ignored):', error.code);
      }
    }
  ...
  /* ============================================
     GOOGLE AUTH — POPUP ONLY (no redirect)
     ============================================ */
  async loginWithGoogle() {
    ...
    // ALWAYS use popup — redirect fails on mobile due to storage partitioning
    try {
  ...
  showError(msg, isPopupBlocked) {
    ...
    if (panel) {
      ...
      // If popup blocked, add a retry button
      if (isPopupBlocked) {
  ...
  updateUI() {
    ...
    if (isLoggedIn) this.hideLoginModal();
